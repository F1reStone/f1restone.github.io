...
- Connected shared Chinese/English UI strings, extracted shared page views, and verified English routes without enabling them for production.
- Fixed browser-language redirects ignoring the i18n switch, four-character locale slug handling, content route filtering, breadcrumb hierarchy, sharing images and SEO output.
- Added untranslated-content notices, locale RSS, llms.txt, strict-consent Umami, working browser checks and an isolated i18n test command.
- Detailed decisions and validation: [migration record](docs/upgrade-as260.md).
